...
-- ============================================================
--  11. AUTH SYNC (Ensure seeded users can log in)
-- ============================================================
DO $$
DECLARE
    r RECORD;
    hashed_password TEXT := crypt('ambulink@2026', gen_salt('bf'));
    new_auth_id UUID;
BEGIN
    FOR r IN SELECT id, email FROM public.users LOOP
        ...
        IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = r.email) THEN
            INSERT INTO auth.users (
                ...
                r.email, hashed_password, NOW(), '{"provider":"email","providers":["email"]}', 
                jsonb_build_object('db_user_id', r.id), NOW(), NOW(), '', '', '', ''
            ) RETURNING id INTO new_auth_id;

            INSERT INTO auth.identities (
                id, user_id, identity_data, provider, provider_id, last_sign_in_at, created_at, updated_at
            ) VALUES (
                gen_random_uuid(), new_auth_id, format('{"sub":"%s","email":"%s"}', new_auth_id, r.email)::jsonb, 
